...
describe('BoxSearch', () => {
  describe('create from block config', () => {
    ...
    it('should populate Map specific values', async () => {
      const search = await Search.fromBlockConfig({
        'search type': 'box',
      ...
      assert(search instanceof BoxSearch, 'Created correct type.');
      assert.equal(search.minLat, '75.9876543', 'Minimum Longitude set.');
      assert.equal(search.maxLat, '76.9876543', 'Maximum Longitude set.');
      assert.equal(search.minLong, '-100.1234568', 'Minimum Latitude set.');
      assert.equal(search.maxLong, '-101.1234568', 'Maximum Latitude set.');
    });
  });
  ...
  describe('to/from URL Search Parameters', () => {
    ...
    it('should read box specific parameters', async () => {
      ...
      search.minLong = -100.123456789;
      search.maxLong = -101.123456789;
      ...
      assert.match(queryStr, /minLong=-100.1234568/, 'Query string includes min long.');
      assert.match(queryStr, /maxLong=-101.1234568/, 'Query string includes max long.');
      const created = await Search.fromQueryString(queryStr);
      assert.deepStrictEqual(created, search, 'Object was parsed from query string correctly.');
    });
  });
  ...
  describe('to CREG URL Search Parameters', () => {
    it('should have Box search parameters', () => {
      ...
      assert.match(queryStr, /%22%3A%5B%5B%5B%22-100.1234568%22%2C%2275.9876543/, 'Query string includes first minLong/minLat point.');
      assert.match(queryStr, /-100.1234568%22%2C%2276.9876543/, 'Query string includes minLong/maxLat point.');
      assert.match(queryStr, /-101.1234568%22%2C%2276.9876543/, 'Query string includes maxLong/maxLat point.');
      assert.match(queryStr, /-101.1234568%22%2C%2275.9876543/, 'Query string includes maxLong/minLat point.');
      assert.match(queryStr, /-100.1234568%22%2C%2275.9876543%22%5D%5D%5D%7D%7D%5D%7D/, 'Query string includes close-the-box minLong/minLat point.');
    });
  });
});
